...
import com.practice.eyepetizer.glide.GlideRoundTransform
...
class VideoDetailAdapter(mContext: Context, data: ArrayList<HomeBean.Issue.Item>) :
    ...
    init {
        textTypeface = Typeface.createFromAsset(GlobleApplication.context.assets, "font/FZLanTingHeiS-L-GB-Regular.TTF")
    }
    ...
    private fun setVideoDetailInfo(data: HomeBean.Issue.Item, holder: ViewHolder) {
        ...
        with(holder) {
            getView<TextView>(R.id.tv_action_favorites).setOnClickListener {
                Toast.makeText(GlobleApplication.context, "喜欢", Toast.LENGTH_SHORT).show()
            }
            getView<TextView>(R.id.tv_action_share).setOnClickListener {
                Toast.makeText(GlobleApplication.context, "分享", Toast.LENGTH_SHORT).show()
            }
            getView<TextView>(R.id.tv_action_reply).setOnClickListener {
                Toast.makeText(GlobleApplication.context, "评论", Toast.LENGTH_SHORT).show()
            }
        }
    }
