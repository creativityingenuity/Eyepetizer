...
import com.practice.eyepetizer.mvp.presenter.VideoDetailPresenter
import com.practice.eyepetizer.mvp.ui.adapter.VideoDetailAdapter
...
import com.practice.eyepetizer.widget.VideoListener
...
class VideoDetailActivity : BaseActivity(), VideoDetailContract.View {
    ...
    private fun saveWatchVideoHistoryInfo(watchItem: HomeBean.Issue.Item) {
        ...
        val historyMap = WatchHistoryUtils.getAll(Constants.FILE_WATCH_HISTORY_NAME, GlobleApplication.context) as Map<*, *>
        for ((key, _) in historyMap) {
            if (watchItem == WatchHistoryUtils.getObject(Constants.FILE_WATCH_HISTORY_NAME,GlobleApplication.context, key as String)) {
                WatchHistoryUtils.remove(Constants.FILE_WATCH_HISTORY_NAME,GlobleApplication.context, key)
            }
        }
        WatchHistoryUtils.putObject(Constants.FILE_WATCH_HISTORY_NAME,GlobleApplication.context, watchItem,"" + mFormat.format(Date()))
    }
